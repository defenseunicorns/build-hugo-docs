import { defineWritePath, getDocumentationFiles } from '../src/fileUtils'

describe('Retreiving files to convert', () => {
  it('Should return a list of files to be converted', async () => {
    const files = await getDocumentationFiles('test/fixtures')
  ...
  it('should accept files as input', async () => {
    const files = await getDocumentationFiles('README.md')
  ...
  it('should ignore requested paths', async () => {
    ...
    const files = await getDocumentationFiles('test/fixtures', [ignorePath])
  ...
  it('Should throw an error of if the input is empty', async () => {
    await expect(getDocumentationFiles()).rejects.toThrow()
  })
})
